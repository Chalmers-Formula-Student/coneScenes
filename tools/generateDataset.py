...
class DatasetGenerator:
    def __init__(self, selected_file: str, lidar_rotation: float = 1.5707, label_every: int = 10):
        self.selected_file: str = selected_file
        self.lidar_rotation: float = lidar_rotation
        self.label_every: int = label_every

    def transform_point_cloud(self, point_cloud: np.ndarray, vx: float, vy: float, yaw_rate: float) -> np.ndarray:
    ...
    def get_local_cones(self, cones, pose,  pc_array: np.ndarray) -> Tuple[List[Tuple[float, float]], float]:
        ...
        for cone in cones:
            ...
            # Check if it contains more than 4 points in the point cloud within a radius of 0.2 units
            if np.sum((pc_array[:, 0] - x_rel) ** 2 + (pc_array[:, 1] - y_rel) ** 2 < 0.5 ** 2) >= 1:
                local_cones.append((x_rel, y_rel))

        return local_cones, yaw

    def read_data(self, global_cones: List[Tuple[float, float]]) -> Generator[Tuple[np.ndarray, List[Tuple[float, float]], Dict[str, Any]], None, None]:
        ...
        while reader.has_next():
            ...
            if topic == POINTCLOUD_TOPIC:
                ...
                pc_array = self.transform_point_cloud(pc_array, vx, vy, yawrate)
                pc_array = pc_array[:, :4]

                cones, yaw = self.get_local_cones(global_cones, pose, pc_array)
                odom = {
    ...
    def gen_data(self, global_cones: List[Tuple[float, float]]):
        ...
        unlabeled_clouds = []
        for i, (pcd, cones, odom) in enumerate(self.read_data(global_cones)):
...
class MapEditor:
    def __init__(self, lidar_rotation: float = 1.5707, label_every: int = 10):
    ...
    def gen_data(self):
        datasetGenerator = DatasetGenerator(self.selected_file, self.lidar_rotation, self.label_every)
...
def parse_args():
    ...
    parser.add_argument("--lidar_rotation", type=float, default=1.5707, help="Lidar rotation")
    parser.add_argument("--label_every", type=int, default=10, help="Label every nth point cloud")
...
def main():
    ...
    editor = MapEditor(args.lidar_rotation, args.label_every)
    editor.run()
